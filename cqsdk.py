...
ClientHello = namedtuple("ClientHello", ("port", "ip"))
...
class CQBot():
    def __init__(self, server_ip, server_port, client_ip, client_port=0, online=True, debug=False):
        self.listeners = []

        self.remote_addr = (server_ip, server_port)
        self.client = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

        self.local_addr = (client_ip, client_port)
    ...
    def server_keepalive(self):
        while True:
            ...
            self.send(ClientHello(port, host))
            time.sleep(10)
...
if __name__ == '__main__':
    try:
        qqbot = CQBot('coolq', 11235, 'qqcaiji', 0)

        @qqbot.listener((RcvdGroupMessage, ))
        def log(message):
            print(message)
            caiji.save(message.text)
